...
/**
 * @file
 * Contains HedleyGithubAuthAuthentication .
 */

class HedleyGithubAuthAuthentication extends \RestfulAccessTokenAuthentication {
  ...
  protected function getUser() {
    ...
    $options = array(
      ...
      'data' => http_build_query(array(
        'client_id' => variable_get('hedley_github_client_id'),
        'client_secret' => variable_get('hedley_github_client_secret'),
        'code' => $request['code'],
      )),
    );
  ...
  protected function createUser($data, $options, $access_token) {
    $fields = array(
      ...
      // Pipe our data so implementing modules so they can use it for example in
      // hook_user_preasve().
      '_github' => array(
  ...
  protected function getEmailFromGithub($options) {
    $data = hedley_github_http_request('user/emails', $options);
